...
## 4. 歩行スプライトシート　`assets/sprites/`（1024×1024・3列×4行）

置くとフィールドの歩行キャラが自動でこの画像に切り替わる（無ければコード描画のちびキャラ）。

| ファイル | キャラ | 状態 |
|---|---|---|
| `oda_walk.png` | オダ | ✅（仮・第2弾で描き直し予定） |
...
現状コードは未参照（**タイトルロール実装時に接続予定**）。第2弾プロンプトで生成。

| ファイル | キャラ | 状態 |
|---|---|---|
| `oda_full.png` 〜 `naiki_full.png` | 主要6人＋内貴 | ⬜ 未導入（踊り子は `enemy/odoriko_fullbody_transparent.png` を流用予定） |
...
- **写真＋本人了承が必要**: キャラ7人の face / walk / full（オダ・いけ・みち・館長・踊り子・坂元・内貴）
- **提供待ち**: BGM・効果音一式
